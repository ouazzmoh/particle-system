...
int main() {
    ...
    string type = "nil";
    Particle *particleToInsert = new Particle(Vecteur(50.0, 50.0, 0.0),
                                              Vecteur(5.0, -5.0, 0.0), 1.0, 1, Vecteur(0.0, 0.0, 0.0), type);
    particleList.push_back(particleToInsert);

    ofstream sim0; ofstream sim1; ofstream sim2; ofstream sim3; ofstream sim4;

    Universe *univ = new Universe(particleList, 2, rCut, lD, epsilon, sigma, 1);

    ofstream out;
    out.open("../../demo/speed");
    stromerVerletPotential(*univ, 20, 0.005, out, "hek");
    out.close();
    return 0;
};
